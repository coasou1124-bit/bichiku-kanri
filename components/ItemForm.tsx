...
export default function ItemForm({ initial, onSubmit, onCancel }: ItemFormProps) {
  ...
  const [alertMode, setAlertMode] = useState<"preset" | "custom">(
    initial && !PRESET_DAYS.includes(initial.alertLead) ? "custom" : "preset"
  );
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div>
          <label className="mb-1 block text-sm font-medium">保管場所</label>
          <input
            list="location-list"
            value={form.location}
      ...
      <div>
        ...
        <select
          value={alertMode === "custom" ? CUSTOM_VALUE : form.alertLead}
          onChange={(e) => {
